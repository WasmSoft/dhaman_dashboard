...
export function PaymentSetupOutcomeSection({
  ...
  return (
    <section className="mt-6 space-y-0" aria-label="ملخص ما بعد الدفع">
      ...
      <OutcomeCard className="rounded-none border-emerald-400/20">
        <OutcomeTitle
          icon={ShieldCheck}
          title={outcome.protection.title}
          className="text-[#4ade80]"
        />
        <p className="mt-3 text-base leading-6 text-[#f1f3fc]">
          {outcome.protection.description}
        </p>
        <PillBadge className="mt-4 border-emerald-400/20 bg-emerald-400/[0.08] text-[#4ade80]">
          <ShieldCheck className="size-[9px]" aria-hidden="true" />
          {outcome.protection.badge}
        </PillBadge>
      </OutcomeCard>
