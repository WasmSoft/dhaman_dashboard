// AR: اختبارات مساعدات عرض المراحل — تتحقق من تحويل المراحل الخام إلى بطاقات وملخصات.
// EN: Milestone display helper tests — verify mapping from raw milestones to cards and summaries.
...
describe("milestone display helpers", () => {
  ...
  it("maps a milestone with missing due date gracefully", () => {
    const mapped = mapMilestoneToWorkspaceMilestone(
      makeMilestone({ dueDate: null }),
    );
    expect(mapped.due).toBe("بدون موعد محدد");
  });
